use squeez::commands::{Handler, git::GitHandler};
...
fn groups_many_modified_files() {
    ...
    for i in 0..25 { lines.push(format!("\tmodified:   src/components/C{}.tsx", i)); }
...
fn git_log_truncates_to_config_limit() {
    let lines: Vec<String> = (0..100).map(|i| format!("abc{:04} subject {}", i, i)).collect();
