...
const LIB_USAGE: &str = include_str!("../../assets/buddy/lib/usage.js");
const LIB_HUD: &str = include_str!("../../assets/buddy/lib/hud.js");
...
/// Statusline shim: renders the squeez status line (duck + 5h/7d/context
/// meters). A previously registered third-party HUD is kept in
/// `<buddy>/hud-command` and used as fallback when `buddy = false` or node is
/// missing — so disabling the buddy never leaves a blank status line.
const STATUSLINE_SHIM: &str = "#!/bin/sh\n\
# squeez buddy statusline — pato + medidores (5h / 7d / contexto)\n\
root=\"$(cd \"$(dirname \"$0\")/..\" && pwd)\"\n\
fallback() {\n\
  [ -f \"$root/hud-command\" ] || exit 0\n\
  exec sh -c \"$(cat \"$root/hud-command\")\"\n\
}\n\
grep -Eq '^[[:space:]]*buddy[[:space:]]*=[[:space:]]*(false|off|0)[[:space:]]*$' \"$root/../config.ini\" 2>/dev/null && fallback\n\
command -v node >/dev/null 2>&1 || fallback\n\
exec node \"$root/statusline/statusline.js\"\n";
